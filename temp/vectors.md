...
Информация будет полезна всем, кто делает первые шаги в ml и data science. В статье много базового кода с объяснениями. Если вы уже знакомы с синтаксом python и numpy, просто пропускайте его. Начнем.
...
Создадим два случайных массива длиной 10 млн скаляров. Чувствуете, работем с Big Data!

```python
vec_a = np.random.randint(100, size=10000000)
vec_b = np.random.randint(200, size=10000000) 
```
Операция ```np.random.randint()``` создает массив из случайных целых чисел. Диапазон подбора чисел лежит в границах от 0 до заданного числа, в нашем случае 100 и 200. Длина масива задается параметром ```size=...```. Полная информация по этой функции и другие способы создания массивов находятся в официальной ![доке numpy](https://numpy.org/doc/stable/reference/random/generated/numpy.random.randint.html). Добавьте в закладки и изучите.

Переведем масивы в списки (list). Нам поможет функция ```tolist()```.  
```python
list_a = vec_a.tolist()
list_b = vec_b.tolist()
```
Проверим, что мы все сделали правильно. Выведем первые 5 объектов вектора "а" и списка "а".  
