...
Вопрос - а зачем нам вообще изучать векторы для работы с данными. Ведь можно просто передовать в python списки, пусть машина работает с ними. Однако, numpy работает с 
векторами (массивами, arrays), гораздо быстрее, чем обычный оператор python со списками и, например, кортежами.
